...
class PoeExample:
    ...
    def select_bot(self):
        ...
        while True:
            ...
            if selection == "0":
                ...
                while True:
                    cateChoice = input("\nYour choice: ")
                    if cateChoice.isdigit() and 1 <= int(cateChoice) <= len(
                        self.categories
                    ):
                ...
                while True:
                    ...
                    if exploreChoice.isdigit() and int(exploreChoice) > 0:
                        available_bots = self.client.explore(
                            categoryName=category, count=int(exploreChoice)
                        )
                        break
                    elif exploreChoice == "all":
                        available_bots = self.client.explore(
                            categoryName=category, explore_all=True
                        )
                        break
                    else:
                        print("Invalid choice. Please select a valid option.\n")
                ...
                while True:
                    ...
                    if query == "":
                        ...
                        while True:
                            ...
                            if exploreChoice.isdigit() and int(exploreChoice) > 0:
                                available_bots = self.client.explore(
                                    search=query, count=int(exploreChoice)
                                )
                                break
                            elif exploreChoice == "all":
                                available_bots = self.client.explore(
                                    search=query, explore_all=True
                                )
                                break
                            else:
                                print("Invalid choice. Please select a valid option.\n")
                    ...
                    if user_query == "":
                        ...
                        while True:
                            ...
                            if userChoice.isdigit() and int(userChoice) > 0:
                                people = self.client.explore(
                                    search=user_query,
                                    entity_type="user",
                                    count=int(userChoice),
                                )
                                break
                            elif userChoice == "all":
                                people = self.client.explore(
    ...
    def chat_thread(self, threads, cursor, page=0):
        ...
        while True:
            ...
            for i, k in enumerate(threads[page]):
                i += 2
                print(
                    f'\033[38;5;121m[{i}]\033[0m Thread {k["chatCode"]} | {k["title"]}'
                )
            if pagination:
            ...
            if choice.isdigit() and 0 <= int(choice) <= len(threads[page]) + 1:
                ...
                if has_next_page:
                    page += 1
                    new_data = self.client.get_chat_history(
                        bot=self.bot, count=20, cursor=cursor
                    )
                    if (
                        new_data["data"][self.bot.lower().replace(" ", "")] == []
                        or new_data["cursor"] == None
                    ):
                        has_next_page = False
                    if page == len(threads):
                        threads.append(
    ...
    def continue_thread(self, bots, message):
        if len(message.split(" ")) == 2 and (
            message.split(" ")[1].isdigit()
            or (
                message.split(" ")[1].startswith("-")
                and message.split(" ")[1][1:].isdigit()
            )
        ):
            ...
            for bot, bot_chats in bots.items():
                for chat in bot_chats:
                    ...
                    if (
                        start_cursor > (page - 1) * pagination
                        and start_cursor <= pagination * page
                    ):
        ...
        print(
            " \033[38;5;121mNo.\033[0m|  \033[38;5;121mChat ID\033[0m   |     \033[38;5;121mChat Code\033[0m       |           \033[38;5;121mBot Name\033[0m             |    \033[38;5;121mChat Title\033[0m"
        )
        print("-" * 90)
        ...
        for index, (bot, bot_chats) in enumerate(new_bots.items()):
            for chat in bot_chats:
                ...
                print(
                    f'[{len(orders)}] | {chat["chatId"]}'
                    + (11 - len(str(chat["chatId"]))) * " "
                    + f'| {chat["chatCode"]} | {bot:<30} | {chat["title"]}'
                )
        ...
        while True:
            if valid_page:
                print("[0] : Return\n" "[>] : Next page\n" "[<] : Previous page\n")
                print(
                    f"You are on page {page} of {start_cursor // pagination + (start_cursor % pagination > 0)}"
                )
            else:
            ...
            if choice == "0":
                ...
                self.chat_with_bot(
                    bot=selected_order[0],
                    chatId=selected_order[1],
                    chatCode=selected_order[2],
                )
                break
            else:
                print("Invalid choice. Please select a valid option.\n")
        ...
        while True:
            ...
            if message == "":
                ...
                try:
                    ...
                    thread = None
                    print(
                        "No threads found. Please type a message to create a new thread first.\n"
                    )
                if thread != None:
                    self.chatId = thread["chatId"]
                    print(f'The selected thread is: {thread["chatCode"]}')
                ...
                if not bots:
                    print(
                        "No history found. Please type a message to create a new thread first.\n"
                    )
                    continue
                else:
                    self.continue_thread(bots, message)
                ...
                if self.chatId is None:
                    ...
                    continue
                previous_messages = self.client.get_previous_messages(
                    bot=self.bot, chatId=self.chatId, get_all=True
                )
                    ...
                    if message["author"] == "human":
                        ...
                        with Live(
                            ...
                            (
                                live.update(
                                    Markdown(
                                        f'\033[38;5;20m{self.bot}\033[0m : {message["text"]}',
                                        code_theme="monokai",
                                    )
                                )
                                if message["contentType"] == "text_markdown"
                                else ""
                            )
                print("\n")
            else:
                ...
                with Live(
                    ...
                    for chunk in self.client.send_message(
                        ...
                        live.update(
                            Markdown(
                                f'\033[38;5;20m{self.bot}\033[0m : {chunk["text"]}',
                                code_theme="monokai",
                            )
                            if content_type == "text_markdown"
                            else ""
                        )
                print("\n")
                if chunk["suggestedReplies"] != []:
                    for reply in range(len(chunk["suggestedReplies"])):
                        print(
                            f"\033[38;2;255;203;107m[Type !suggest {reply+1}] : {chunk['suggestedReplies'][reply]}\033[0m\n"
                        )
                if self.chatId is None:
