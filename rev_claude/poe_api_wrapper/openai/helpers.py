...
async def __progressive_summarize_text(
    ...
    words = [
        ps.stem(word) for word in word_tokenize(text.lower()) if word not in stop_words
    ]
                ...
                if item["type"] == "text":
                    ...
                    if "image_url" in item:
                        if isinstance(item["image_url"], str):
                            ...
                            logger.error(
                                f"Invalid image URL format: {item['image_url']}"
                            )
...
async def __add_tools_prompt(messages):
    ...
    messages = [
        message
        for message in messages
        if "tool_calls" not in message and message["role"] != "tool"
    ]
    return messages
