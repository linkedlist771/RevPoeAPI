...
class AsyncPoeApi:
    ...
    def __init__(self, tokens: dict = {}, proxy: list = [], auto_proxy: bool = False):
        ...
        self.client.cookies.update(
            {"p-b": self.tokens["p-b"], "p-lat": self.tokens["p-lat"]}
        )
    ...
    async def create(self):
        ...
        if self.proxy != [] or self.auto_proxy == True:
            ...
            raise ValueError(
                "Please provide a valid proxy list or set auto_proxy to False"
            )
    ...
    async def load_bundle(self):
        try:
            ...
            logger.warning(
                "Failed to get formkey from bundle. Please provide a valid formkey manually."
                if self.formkey == ""
    ...
    async def select_proxy(self, proxy: list, auto_proxy: bool = False):
        ...
        for p in range(len(proxies)):
            try:
                ...
                logger.info(
                    f"Connection failed with {proxies[p]}. Trying {p+1}/{len(proxies)} ..."
                )
        ...
        try:
            ...
            if isinstance(e, ReadTimeout):
                if query_name == "SendMessageMutation":
                    logger.error(
                        f"Failed to send message {variables['query']} due to ReadTimeout"
                    )
                    raise e
                else:
                    logger.error(
                        f"Automatic retrying request {query_name} due to ReadTimeout"
                    )
                    return await self.send_request(
                        path, query_name, variables, file_form
                    )
            ...
            if (
                ...
                return await self.send_request(
                    path, query_name, variables, file_form, ratelimit=ratelimit + 1
                )
    ...
    async def connect_ws(self, timeout=20):
        ...
        self.ws = await self.loop.run_in_executor(
            ...
            lambda: websocket.WebSocketApp(
                ...
                on_close=lambda ws, close_status_code, close_message: self.on_ws_close(
                    ws, close_status_code, close_message
                ),
            ),
        )
        ...
        try:
            ...
            for data in ws_data["messages"]:
                ...
                if (
                    ...
                    and data["messageAdded"]["author"] == "human"
                ):
                    return

                chat_id: int = int(
                    payload.get("unique_id")[(len(subscriptionName) + 1) :]
                )
                ...
                if chat_id in self.message_queues:
                    ...
                    #     }
                    # )
                    # await self.message_queues[chat_id].put(
                    #     {
                    #             "data": data,
                    #             "subscription": subscriptionName,
                    #         }
                    ...
                    if subscriptionName == "messageAdded":
                        self.active_messages[chat_id] = data["messageAdded"][
                            "messageId"
                        ]
                    return
    ...
    async def get_chat_history(
        self, bot: str = None, count: int = None, interval: int = 50, cursor: str = None
    ):
                ...
                while response_json["data"]["filteredChats"]["pageInfo"]["hasNextPage"]:
                    ...
                    for edge in edges:
                        ...
                        try:
                            ...
                            logger.debug(str(e))
                            pass
                    cursor = response_json["data"]["filteredChats"]["pageInfo"][
                        "endCursor"
                    ]
                    chat_bots["cursor"] = cursor
                if not response_json["data"]["filteredChats"]["pageInfo"][
                    "hasNextPage"
                ]:
                    chat_bots["cursor"] = None
        return chat_bots

    async def get_threadData(
        self, bot: str = "", chatCode: str = None, chatId: int = None
    ):
        id = None
        title = None
    ...
    async def get_botInfo(self, handle: str):
        ...
        response_json = await self.send_request(
            "gql_POST", "HandleBotLandingPageQuery", {"botHandle": handle}
        )
        if response_json["data"] == None and response_json["errors"]:
            raise ValueError(
    ...
    async def retry_message(
        ...
        while (
            ...
            and len(self.active_messages) > self.MAX_CONCURRENT_MESSAGES
        ):
            await asyncio.sleep(0.01)
            timer += 0.01
            if timer > timeout:
        ...
        msgPrice = response_json["data"]["chatOfCode"]["defaultBotObject"][
            "messagePointLimit"
        ]["displayMessagePointPrice"]
        ...
        response_json = await self.send_request(
            ...
            {"messageId": bot_message_id, "messagePointsDisplayPrice": msgPrice},
        )
        if response_json["data"] == None and response_json["errors"]:
            logger.error(
                f"Failed to retry message {bot_message_id} of Thread {chatCode}. Raw response data: {response_json}"
            )
        else:
            logger.info(
                f"Message {bot_message_id} of Thread {chatCode} has been retried."
            )
        ...
        while True:
            ...
            if ws_data["subscription"] == "messageAdded" or title:
                ...
                if (
                    response["author"] == "pacarana"
                    and response["text"].strip() == last_text.strip()
                ):
                    response["response"] = ""
                elif response["author"] == "pacarana" and (
                    last_text == "" or bot != "web-search"
                ):
                ...
                if response["state"] == "complete":
                    if suggest_replies:
                        if (
                            suggest_attempts > 0
                            and len(response["followupActions"]) <= 6
                        ):
                            actions = response["followupActions"]
                            suggestedReplies = [
                                action["bodyText"] for action in actions
                            ]
                            suggest_attempts -= 1
                            await asyncio.sleep(1)
                            continue
    ...
    async def send_message(
        ...
        if chatId == None and chatCode == None:
            try:
                ...
                message_data = await self.send_request(
                    apiPath, "SendMessageMutation", variables, file_form
                )
                ...
                if bot not in self.current_thread:
                    ...
                    self.current_thread[bot].append(
                        {
                            "chatId": chatId,
                            "chatCode": chatCode,
                            "id": message_data["id"],
    ...
    async def cancel_message(self, chunk: dict):
        ...
        await self.send_request(
            "gql_POST", "StopMessage_messageCancel_Mutation", variables
        )
    ...
    async def purge_conversation(
        ...
        if del_all == True:
            while True:
                ...
                edges = response_json["data"]["chatOfCode"]["messagesConnection"][
                    "edges"
                ]
    ...
    async def delete_chat(
        self, bot: str, chatId: any = None, chatCode: any = None, del_all: bool = False
    ):
        bot = bot_map(bot)
        try:
            temp = await self.get_chat_history(bot=bot)
        ...
        if del_all == True:
            ...
            for chat in chatdata:
                await self.send_request(
                    "gql_POST", "DeleteChat", {"chatId": chat["chatId"]}
                )
                        ...
                        if bot in self.current_thread:
                            for thread in range(len(self.current_thread[bot])):
                                if self.current_thread[bot][thread]["chatId"] == chatId:
                                    ...
                                    break
                        await self.send_request(
                            "gql_POST", "DeleteChat", {"chatId": chatId}
                        )
                        logger.info(f"Chat {chatId} deleted")
                else:
                    if chat["chatCode"] == chatCode:
    ...
    async def get_previous_messages(
        ...
        if get_all:
            while edges:
                ...
                response_json = await self.send_request(
                    "gql_POST", "ChatListPaginationQuery", variables
                )
    ...
    async def complete_profile(self, handle: str = None):
        ...
        await self.send_request(
            "gql_POST", "NuxInitialModal_poeSetHandle_Mutation", variables
        )
        await self.send_request("gql_POST", "MarkMultiplayerNuxCompleted", {})

    async def get_available_knowledge(
        self, botName: str, count: int = 10, get_all: bool = False
    ):
        ...
        response = await self.send_request(
            "gql_POST", "BotKnowledgeSourcesModalPaginationQuery", new_variables
        )
        ...
        if edges:
            for edge in edges:
                if edge["node"]["title"] not in sources_ids:
                    sources_ids[edge["node"]["title"]] = [
                        edge["node"]["knowledgeSourceId"]
                    ]
                else:
                    sources_ids[edge["node"]["title"]].append(
                        edge["node"]["knowledgeSourceId"]
                    )
                total_sources = edge["cursor"]
            while len(sources_ids) < count or get_all:
                if response["data"]["node"]["knowledgeSourceConnection"]["pageInfo"][
                    "hasNextPage"
                ]:
                    cursor = response["data"]["node"]["knowledgeSourceConnection"][
                        "pageInfo"
                    ]["endCursor"]
                else:
                    break
                ...
                if edges:
                    edges = response["data"]["node"]["knowledgeSourceConnection"][
                        "edges"
                    ]
                    for edge in edges:
                        if edge["node"]["title"] not in sources_ids:
                        ...
                        total_sources = edge["cursor"]
        logger.info(
            f"Found {len(sources_ids)} unique knowledge sources out of {int(total_sources)+1} sources from {botName}"
        )
        return sources_ids
        ...
        if text_knowledge != []:
            for text in text_knowledge:
                if text != {} and "title" not in text and "content" not in text:
                    error_msg = (
                        f"Invalid text knowledge {text}. \nPlease make sure the text knowledge is in the format of "
        ...
        if file_path != []:
            for path in file_path:
                ...
                response = await self.send_request(
                    "gql_upload_POST",
                    "Knowledge_CreateKnowledgeSourceMutation",
                    {"sourceInput": {"file_upload": {"attachment": "file"}}},
                    file_form,
                    knowledge=True,
                )
                if response["data"]["knowledgeSourceCreate"]["status"] != "success":
    ...
    async def edit_knowledge(
        self, knowledgeSourceId: int, title: str = None, content: str = None
    ):
        variables = {
        ...
        response = await self.send_request(
            "gql_POST", "Knowledge_EditKnowledgeSourceMutation", variables
        )
    ...
    async def get_citations(self, messageId: int):
        ...
        response = await self.send_request(
            "gql_POST", "MessageCitationSourceModalQuery", variables
        )
    ...
    async def get_available_creation_models(self):
        ...
        models = {
            "text": [
                bot["model"]
                for bot in models_data
                if not bot["isImageGen"] and not bot["isVideoGen"]
            ],
    ...
    async def create_bot(
        ...
        knowledgeSourceIds: dict = {},
        allowRelatedBotRecommendations=True,
        inviteCodeReqHash=None,
    ):

        if not re.match("^[a-zA-Z0-9_.-]{4,20}$", handle):
            raise ValueError(
                "Invalid handle. Should be unique and use 4-20 characters, including letters, numbers, dashes, periods and underscores."
            )
        ...
        if base_model not in bot_models:
            raise ValueError(
                f"Invalid base model {base_model}. Please choose from {bot_models}"
            )
        ...
        if botCategory:
            if botCategory in (1, 4):
                if base_model not in botCategories["text"]:
                    raise ValueError(
                        f"Invalid base model {base_model} for category {botCategory}. Please choose from {botCategories['text']}"
                    )
            elif botCategory == 2:
                if base_model not in botCategories["image"]:
                    raise ValueError(
                        f"Invalid base model {base_model} for category {botCategory}. Please choose from {botCategories['image']}"
                    )
            elif botCategory == 3:
                if base_model not in botCategories["video"]:
                    raise ValueError(
                        f"Invalid base model {base_model} for category {botCategory}. Please choose from {botCategories['video']}"
                    )
            else:
                raise ValueError(
                    f"Invalid bot category {botCategory}. Please choose from 1 (Prompt bot), 2 (Image generation), 3 (Video generation) or 4 (Role play)"
                )
        else:
            botCategory = (
                1
                if base_model in botCategories["text"]
                else (
                    2
                    if base_model in botCategories["image"]
                    else 3 if base_model in botCategories["video"] else None
                )
            )
        ...
        if knowledgeSourceIds != {}:
            sourceIds = [
                item for sublist in knowledgeSourceIds.values() for item in sublist
            ]
        else:
            sourceIds = []
        ...
        if result["status"] != "success":
            logger.error(
                f"Poe returned an error while trying to create a bot: {result['status']}"
            )
        else:
    ...
    async def get_botData(self, handle):
        ...
        try:
            response_json = await self.send_request(
                "gql_POST", "LayoutRightSidebarQuery", variables
            )
    ...
    async def edit_bot(
        ...
        temperature=None,
        customMessageLimit=None,
        knowledgeSourceIdsToAdd: dict = {},
        knowledgeSourceIdsToRemove: dict = {},
        messagePriceCc=None,
        shouldCiteSources=True,
        allowRelatedBotRecommendations=True,
    ):
        ...
        if knowledgeSourceIdsToAdd != {}:
            addIds = [
                item for sublist in knowledgeSourceIdsToAdd.values() for item in sublist
            ]
        else:
            addIds = []
        if knowledgeSourceIdsToRemove != {}:
            removeIds = [
                item
                for sublist in knowledgeSourceIdsToRemove.values()
                for item in sublist
            ]
        else:
            removeIds = []
        ...
        if result["status"] != "success":
            logger.error(
                f"Poe returned an error while trying to edit a bot: {result['status']}"
            )
        else:
            if new_handle and handle != new_handle:
                ...
                logger.info(
                    f"Bot edited successfully | New handle from {handle} to {new_handle}"
                )
            else:
    ...
    async def explore(
        self,
        categoryName: str = "defaultCategory",
        search: str = None,
        entity_type: str = "bot",
        count: int = 50,
        ...
        if entity_type not in ["bot", "user"]:
            raise ValueError(
                f"Entity type {entity_type} not found. Make sure the entity type is either bot or user."
            )
        if (
            categoryName != "defaultCategory"
            and categoryName not in await self.get_available_categories()
        ):
            raise ValueError(
                f"Category {categoryName} not found. Make sure the category exists before exploring."
            )
        bots = []
        if search == None:
            ...
            if len(result["data"][connectionType]["edges"]) == 0:
                if not explore_all:
                    if entity_type == "bot":
                        ...
                        logger.info(
                            f"No more users could be explored, only {len(bots)} users found."
                        )
                return bots
            if search == None:
                new_cursor = result["data"][connectionType]["edges"][-1]["cursor"]
    ...
    async def share_chat(
        self, bot: str, chatId: int = None, chatCode: str = None, count: int = None
    ):
        bot = bot_map(bot)
        chatdata = await self.get_threadData(bot, chatCode, chatId)
        chatCode = chatdata["chatCode"]
        ...
        variables = {
            "chatId": chatId,
            "messageIds": message_ids if count == None else message_ids[:count],
        }
        response_json = await self.send_request(
            "gql_POST", "ShareMessageMutation", variables
        )
    ...
    async def import_chat(self, bot: str = "", shareCode: str = ""):
        ...
        if response_json["data"]["continueChatFromPoeShare"]["status"] == "success":
            ...
            chatCode = response_json["data"]["continueChatFromPoeShare"]["messages"][0][
                "node"
            ]["chat"]["chatCode"]
    ...
    async def create_group(self, group_name: str = None, bots: list = []):
        ...
        if group_name in self.groups:
            raise ValueError(
                f"Group {group_name} already exists. Please try again with a different group name."
            )
    ...
    async def delete_group(self, group_name: str):
        if group_name not in self.groups:
            raise ValueError(
                f"Group {group_name} not found. Make sure the group exists before deleting."
            )
    ...
    async def get_group(self, group_name: str):
        if group_name not in self.groups:
            raise ValueError(
                f"Group {group_name} not found. Make sure the group exists before getting."
            )
    ...
    async def send_message_to_group(
        ...
        if group_name not in self.groups:
            raise ValueError(
                f"Group {group_name} not found. Make sure the group exists before sending message."
            )
        ...
        if preset_history == "":
            ...
            preset_log = await self.load_group_history(file_path=preset_history)[
                "group_data"
            ]["conversation_log"]
            if preset_log != []:
        ...
        if autoplay == False:
            ...
            try:
                previous_text = (
                    self.groups[group_name]["conversation_log"][-1]
                    .split(":")[1]
                    .strip()
                )
            except:
        ...
        async for _ in range(max_turns):
            ...
            async for chunk in self.send_message(
                current_bot["bot"], next_message, chatCode=current_bot["chatCode"]
            ):
            ...
            self.groups[group_name]["conversation_log"].append(
                f"{current_bot['name']} : {chunk['text']}\n"
            )
            ...
            if current_bot["name"] in self.groups[group_name]["dual_lock"]:
                ...
                if len(self.groups[group_name]["conversation_log"]) > 10:
                    current_bot["bot_log"] = self.groups[group_name][
                        "conversation_log"
                    ][-10:]
                else:
                ...
                for index in range(len(self.groups[group_name]["dual_lock"])):
                    if self.groups[group_name]["dual_lock"][index] != prev_bot["name"]:
                        self.groups[group_name]["dual_lock"][index] = current_bot[
                            "name"
                        ]
                        break
