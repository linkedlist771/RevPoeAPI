...
class PoeApi:
    ...
    def send_message(
        ...
        self.retry_attempts = 3
        timer = 0
        while None in self.active_messages.values() and (
            len(self.active_messages) > self.MAX_CONCURRENT_MESSAGES
        ):
            sleep(0.01)
            timer += 0.01
            if timer > timeout:
        ...
        if chatId == None and chatCode == None:
            try:
                ...
                message_data = self.send_request(
                ...
                if message_data["data"] == None and message_data["errors"]:
                    ...
                    if status == "success" and file_path != []:
                        ...
                        for chunk in self.send_message(
                            bot,
                            message,
                            chatId,
                            chatCode,
    ...
    def create_bot(
        ...
        result = self.send_request("gql_POST", "PoeBotCreate", variables)["data"][
            "poeBotCreate"
        ]
    ...
    def edit_bot(
        ...
        result = self.send_request("gql_POST", "PoeBotEdit", variables)["data"][
            "poeBotEdit"
        ]
        if result["status"] != "success":
            logger.error(
    ...
    def explore(
        ...
        if (
            ...
            and categoryName not in self.get_available_categories()
        ):
            raise ValueError(
    ...
    def send_message_to_group(
        ...
        if preset_history == "":
            ...
            preset_log = self.load_group_history(file_path=preset_history)[
                "group_data"
            ]["conversation_log"]
            if preset_log != []:
