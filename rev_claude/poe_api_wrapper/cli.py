...
def main():
    parser = argparse.ArgumentParser(
        prog="poe",
        description="Poe.com wrapper. Have free access to ChatGPT, Claude, Llama, Gemini, Google-PaLM and more!",
    )
    ...
    parser.add_argument(
        ...
        "--version",
        action="version",
        version="v" + metadata.version("poe-api-wrapper"),
    )
    args = parser.parse_args()
    tokens = (
        {"p-b": args.b, "p-lat": args.lat, "formkey": args.f}
        if args.f
        else {"p-b": args.b, "p-lat": args.lat}
    )
